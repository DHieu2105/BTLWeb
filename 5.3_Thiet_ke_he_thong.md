...
│  Bootstrap 5 | Responsive Design | Real-time Validation    │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
│              MVC CONTROLLERS & ROUTING                       │
│  Admin | Auth | GiaoVien | KhoaHoc | LopHoc | Etc.         │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
│              SERVICES & BUSINESS LOGIC                       │
│  AuthService | UserInfoService | PasswordHasher            │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
│           ENTITY FRAMEWORK CORE (ORM)                        │
│  TtanContext | DbSets | Relationships & Validation         │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
│            SQL SERVER DATABASE                              │
│  12+ Tables | Relationships | Constraints                  │
└─────────────────────────────────────────────────────────────┘
```
...
**Luồng xác thực:**
```
User → Nhập Username/Password → AuthController.Login()
      → AuthService.AuthenticateAsync() → Kiểm tra Database
      → PasswordHasher.Verify() → So sánh mật khẩu
      → Tạo Claims (Role, Username, Name, ID) → SignInAsync()
      → Redirect theo Role (Admin Dashboard, Staff Dashboard, etc.)
```
...
```
┌──────────────┐         ┌──────────────┐         ┌──────────────┐
│  TaiKhoan    │         │  TrungTam    │         │  PhongHoc    │
│──────────────│         │──────────────│         │──────────────│
│ Username (PK)│         │ MaTrungTam   │         │ MaPhong (PK) │
│ Password     │         │ TenTrungTam  │         │ TenPhong     │
│ Role         │         │ DiaChi       │         │ MaTrungTam   │
└──────────────┘         │ Sdt          │         │ SucChua      │
       │                 └──────────────┘         └──────────────┘
       │                        ▲ (1)                     │
       │                        │                         │ (1)
   ...
   (1) │ (M)      (1) │                (M) │             (M)│
┌──────▼──────────┐    │    ┌──────────────▼─┐       ┌──────▼────────┐
│   GiaoVien      │    │    │    HocVien     │       │    ThietBi     │
│─────────────────│    │    │────────────────│       │────────────────│
│ MaGv (PK)       │    │    │ MaHocVien (PK) │       │ MaThietBi (PK) │
│ Ten             │    │    │ HoVaTen        │       │ TenThietBi     │
│ Sdt             │    │    │ Sdt            │       │ SoLuong        │
│ ChuyenMon       │    │    │ NgayDangKi     │       └────────────────┘
│ GioiTinh        │    │    │ GioiTinh       │              (1)
│ MaTrungTam      │    │    │ MaNv (FK)      │               │
│ MaKhoaHoc       │    │    │ Username       │               │ (M)
│ Username        │    │    │ Password       │               │
└─────────────────┘    │    └────────────────┘               │
       │ (M)           │            │ (M)                     │
       │               │            ├────────┐               │
   (1) │           (1) │        (M) │        │ (M)           │
┌──────▼──────────┐    │    ┌───────▼────┐   │   ┌──────────▼──────┐
│    LopHoc       │◄───┼────│  KhoaHoc   │   │   │  PhongHoc_ThietBi│
│─────────────────│    │    │────────────│   │   │───────────────────│
│ MaLop (PK)      │    │    │ MaKhoaHoc  │   │   │ MaPhong (PK,FK)  │
│ TenLop          │    │    │ TenKhoaHoc │   │   │ MaThietBi(PK,FK) │
│ MaGv (FK)       │    │    │ ThoiLuong  │   │   │ SoLuong          │
│ MaPhong (FK)    │    │    │ HocPhi     │   │   └──────────────────┘
│ MaKhoaHoc (FK)  │    │    │ MaTrungTam │   │
│ NgayKhai        │    │    └────────────┘   │
│ NgayKet         │    │          │ (1)      │
└─────────────────┘    │          │          │
       │ (M)           │      (M) │      (M) │
   ...
   (M) │               │    ┌─────▼──┐  ┌───▼────────┐
       ├──────────┐    │    │ DangKi │  │  KetQua    │
       │          │    │    │────────│  │────────────│
   (M) │      (M) │    │    │MaHocVien│  │MaHocVien   │
   (1) │          │    │    │MaKhoaHoc│  │MaKhoaHoc   │
┌──────▼──────────┼────┼────│NgayDangKi│ │DiemListening│
│   HocVien_LopHoc│    │    └─────────┘  │DiemReading  │
│──────────────────│    │         (1)     │DiemTong(Calc)
│MaHocVien(PK,FK) │    │          │      └─────────────┘
│MaLop(PK,FK)     │    │          │
└──────────────────┘    │      (M) │
...
### **Quy trình Đăng nhập:**
```
Nhập thông tin → Xác thực mật khẩu → Tạo Session 
→ Điều hướng theo Role → Truy cập Dashboard
```

### **Quy trình Quản lý Sinh viên:**
```
Admin/Staff → Danh sách SV → Tìm kiếm/Phân trang 
→ Thêm/Sửa/Xóa → Lưu DB
```

### **Quy trình Nhập Điểm:**
```
Giáo viên → Chọn Lớp → Chọn Sinh viên → Nhập Listening, Reading 
→ Tính Tổng tự động → Lưu DB
```

### **Quy trình Báo cáo:**
```
Chọn loại báo cáo → Chọn ngày/tháng → Tính toán dữ liệu 
→ Xuất PDF/Excel
```
